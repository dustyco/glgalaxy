...
float* high(float* a, float* b) { if(a>b) return a; else return b; }

void loadTextures() {
	
	//load and decode
	std::vector<unsigned char> buffer, image;
	LodePNG::loadFile(buffer, "transparent.png"); //load the image file with given filename
	...
	glTexParameteri(
		...
		GL_LINEAR);
	std::cout << "Texture prepared\n";
	
	glTexImage2D(
		GL_TEXTURE_2D,		//GLenum target,
		0,			//GLint  level,
		GL_RGBA,		//GLint  internalFormat,
		decoder.getWidth(),	//GLsizei  width,
		decoder.getHeight(),	//GLsizei  height,
		0,			//GLint  border,
		GL_RGBA,		//GLenum  format,
		GL_UNSIGNED_BYTE,	//GLenum  type,
		&image[0]);
	std::cout << "Finished loading image\n";
	}
	...
	*/	
	...
	while (win.IsOpened()) {
		...
		for( long i = 0; i<points; i++ ) {
			...
			float color = 400.0 / *high( &scene[setA][i][2],&scene[setB][i][2] );
		...
		glColor3f(1,1,1);
		glTexCoord2i( 0,0 );
		glVertex2f( width/2-200, height/2-200 );
		glTexCoord2i( 1,0 );
		glVertex2f( width/2+200, height/2-200 );
		glTexCoord2i( 1,1 );
		glVertex2f( width/2+200, height/2+200 );
