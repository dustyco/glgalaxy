...
int main(int argc, const char** argv) {
	...
	*/	
		...
		jitter[pointsRead] = (float)rand()/(float)RAND_MAX;
	...
	*/	
		...
		//	if( controls.IsKeyDown(sf::Key::CTRL)    ) localM[1]++;
			...
			float speedUp = controls.GetJoystickAxis(JOYID,sf::Joy::AxisZ)*0.045+5.5;
			float speedDo = controls.GetJoystickAxis(JOYID,sf::Joy::AxisR)*0.045+5.5;
			float speed = speedDo/speedUp;
			//printf( "speed: %.4f,%.4f \t %.4f\n",speedUp,speedDo,speed );
			//std::cout<<"things\n";
			localM[0]*=speed;
			localM[1]*=speed;
			localM[2]*=speed;
		...
		glBegin(GL_LINES);
		float sep = 1000;
		for( int eye = 0; eye<2; eye++ ) for( long i = 0; i<points; i++ ) {
			pz = scene[setA][i][2];
			if( eye==0 ) {
				px = (scene[setA][i][0]-sep/2)    / pz * height + width/4;
				py =  scene[setA][i][1]           / pz * height + height/2;
				lastx = (scene[setB][i][0]-sep/2) / scene[setB][i][2] * height + width/4;
				lasty =  scene[setB][i][1]        / scene[setB][i][2] * height + height/2;
				}
			else {
				px = (scene[setA][i][0]+sep/2)    / pz * height + width/4*3;
				py =  scene[setA][i][1]           / pz * height + height/2;
				lastx = (scene[setB][i][0]+sep/2) / scene[setB][i][2] * height + width/4*3;
				lasty =  scene[setB][i][1]        / scene[setB][i][2] * height + height/2;
				}
			...
			float r,g,b;
			//if(eye==0) { r = color; g = 0; b = 0;     }
			//else       { r = 0;     g = 0; b = color; }
			r = color*(lum*-.6+1);
			g = color*.6;
			b = color*lum;
			
			//if( pz>10 && (  (px>0 && px<width && py>0 && py<height) || (lastx>0 && lastx<width && lasty>0 && lasty<height)  ) ) {
			if( py>0 && py<height && pz>10 ) {
				if( eye==1 && px>width/2 && px<width ) {
					if(mag>1 && mag>.5/color) {
						glColor3f( r/mag,g/mag,b/mag );
						glVertex2f( px, py );
		...
		glBegin(GL_LINES);
		glColor3f( 0,1,0 );
		glVertex2f( width/4, height/2 );
		glVertex2f( width/4, height/2+4 );
		glVertex2f( width/4*3, height/2 );
		glVertex2f( width/4*3, height/2+4 );
		glEnd();
